...
import Value
import Value.Export.Debug (termsRepToJSONWithAddr, valToStringTermsRep)

reduceDisj :: Disj -> ValAddr -> RM Val
reduceDisj d addr = traceSpanTM "reduceDisj" addr emptySpanValue $ do
  -- We have to reduce all disjuncts because they might be generated by merging one disjunction with another value.
  nd <- vtmapM (applyAddrFOnVal reduce) addr d
  ...
  traceSpanTM
    ...
    $ do
      ...
      debugInstStr
        ...
        addr
        ( do
            flattenedRep <- valToStringTermsRep (mkDisjVN flattened)
            rep <- valToStringTermsRep (mkDisjVN final)
      ...
      if
        | null final.dsjDisjuncts ->
            let
             ...
             in
              if
                | length noVals == length flattened.dsjDisjuncts -> return VNoVal
                | not (null bottoms) -> return $ value $ head bottoms
                | otherwise ->
        ...
        | length final.dsjDisjuncts == 1 && null (dsjDefIndexes final) -> return $ value $ head (toList final.dsjDisjuncts)
 ...
 where
  ...
  go (accIs, accDisjs) (idx, v) = do
    case v of
      ...
      --         else updateDisjuncts (accIs, accDisjs) (idx, f.val)

      -- FIXIME: is the following case possible?
      IsStruct Struct{stcEmbedVal = Just ev} -> updateDisjuncts (accIs, accDisjs) (idx, mkValVN ev)
          ...
          let v = dstValue term
           in if
                ...
                | hasMarked ->
                    maybe
                      v
                      (\d -> setVNodeValue (VDisj $ d{dsjDefIndexes = []}) v)
                      (rtrDisj (value v))
                      : accDisjuncts
