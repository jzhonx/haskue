...
data ReducedSignal = ReducedSignal
  ...
  , depMatchByPrefix :: !Bool
  }
  deriving (Show, Generic, NFData)

instance ShowWTIndexer ReducedSignal where
  tshow ReducedSignal{addr, depGroup, createdWithRCResolver, depMatchByPrefix} = do
    addrT <- tshow addr
    depGroupT <- tshow depGroup
    return $
      T.pack $
        printf
          "ReducedSignal {addr:%s,depGroup:%s,RCResolver:%s,depMatch:%s}"
          addrT
          depGroupT
          (show createdWithRCResolver)
          (show depMatchByPrefix)
